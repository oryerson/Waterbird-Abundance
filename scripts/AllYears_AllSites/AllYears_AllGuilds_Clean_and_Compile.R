# This is a script to further clean and complile data for all estuaries 
# that includes all sites and all years. 
#
# Author: Owen Ryerson
# Date created: May 4 2019
# Date last modified: May 5 2019
#
rm(list=ls())
...
if(Sys.info()[6]!="abramfleishman"){
  ...
  cruz<-read_csv("/Users/owenryerson/Desktop/Birds!/R data Processing/CSV/LaCruz/LaCruz_Guilds_AllYearsAllSites.csv")# read_csv keeps mading some column's values= na
  tast<-read_csv("/Users/owenryerson/Desktop/Birds!/R data Processing/CSV/Tastiota/Tastiota_Guilds_AllYearsAllSites.csv")
  card<-read_csv("/Users/owenryerson/Desktop/Birds!/R data Processing/CSV/Cardonal/Cardonal_Guilds_AllYearsAllSites.csv")
  out_dir<-'/Users/owenryerson/Desktop/Birds!/R data Processing/CSV'
}else{
  # paths for abram
...
cruz$monthnum <- months.POSIXt(cruz$DateTime, abbreviate = TRUE)
cruz$monthnum <- match(cruz$monthnum, month.abb) # created numeric months

card$monthnum <- months.POSIXt(card$DateTime, abbreviate = TRUE)
card$monthnum <- match(card$monthnum, month.abb) # created numeric months

tast$monthnum <- months.POSIXt(tast$DateTime, abbreviate = TRUE)
tast$monthnum <- match(tast$monthnum, month.abb) # created numeric months



# clean 
cruz1<- cruz
cruz1$Precip<-gsub("%|<","",cruz$Precip, fixed=F) # Removes % and < from precip column
...
cruz_clean<-cruz %>%
  ...
  select(Estuary,Point=PointLoc,date,TimeStart,year_season,month=monthnum,month_of_season,day_of_season,day_of_season2,hour,tide_height,tide_dir,Wind,Cloud,TempF,guild,Migratory,landbird,Species,Count)
...
table(tast$Precip)
table(tast$Wind )
table(tast$PointLoc )  # I am almost positive that Camp is the same as town, they are not ever on the same day. 
...
tast_clean<-tast %>%
  mutate(date=ymd(DateTime), # Make date
         ...
         TempF=ifelse(is.na(TempF),round(mean(TempF,na.rm=T)),TempF)) %>%
  filter(PointLoc!="restaurant") %>%
  select(Estuary,Point=PointLoc,date,TimeStart,year_season,month=monthnum,month_of_season,day_of_season,day_of_season2,hour,tide_height,tide_dir,Wind,Cloud=Cloudcover,TempF,guild,Migratory,landbird,Species,Count)
